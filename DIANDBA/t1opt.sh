...
#SBATCH --job-name=dCALC.TADF.DIANDBA.t1opt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/DIANDBA/t1opt.in /data/mewes/CALC/TADF/DIANDBA/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/DIANDBA/t1opt.out ] && cp /data/mewes/CALC/TADF/DIANDBA/t1opt.out /data/mewes/CALC/TADF/DIANDBA/t1opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save t1opt.in /data/mewes/CALC/TADF/DIANDBA/t1opt.out t1opt.out.plots
else
$QC/bin/qchem t1opt.in /data/mewes/CALC/TADF/DIANDBA/t1opt.out
fi

...
#SBATCH --job-name=dCALC.tadf.ANDBA-MeEt-MeCz.s1opt.in
#SBATCH -t 0-12:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.in /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/*.mol .

# Backup old out file is existing 
[ -e /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.out ] && cp /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.out /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save s1opt.in /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.out s1opt.out.plots
else
$QC/bin/qchem s1opt.in /data/mewes1/CALC/tadf/ANDBA-MeEt-MeCz/s1opt.out
fi

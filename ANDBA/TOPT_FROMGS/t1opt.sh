...
#SBATCH --job-name=dCALC.TADF.ANDBA.TOPT_FROMGS.t1opt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.in /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.out ] && cp /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.out /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 t1opt.in /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.out t1opt.out.plots
else
$QC/bin/qchem -np 1 t1opt.in /data/mewes/CALC/TADF/ANDBA/TOPT_FROMGS/t1opt.out
fi

...
#SBATCH --job-name=dCALC.TADF.ANDBA.s2opt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/ANDBA/s2opt.in /data/mewes/CALC/TADF/ANDBA/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/ANDBA/s2opt.out ] && cp /data/mewes/CALC/TADF/ANDBA/s2opt.out /data/mewes/CALC/TADF/ANDBA/s2opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save s2opt.in /data/mewes/CALC/TADF/ANDBA/s2opt.out s2opt.out.plots
else
$QC/bin/qchem s2opt.in /data/mewes/CALC/TADF/ANDBA/s2opt.out
fi

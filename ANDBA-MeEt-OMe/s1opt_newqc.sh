...
#SBATCH --job-name=.data.mewes.CALC.TADF.ANDBA-OMe+MeEt.s1opt_newqc.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.in /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.out ] && cp /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.out /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save s1opt_newqc.in /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.out s1opt_newqc.out.plots
else
$QC/bin/qchem s1opt_newqc.in /data/mewes/CALC/TADF/ANDBA-OMe+MeEt/s1opt_newqc.out
fi

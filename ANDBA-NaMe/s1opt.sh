...
#SBATCH --job-name=dCALC.TADF.ANDBA-NaMe.s1opt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.in /data/mewes1/CALC/TADF/ANDBA-NaMe/*.mol .

# Backup old out file is existing 
[ -e /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.out ] && cp /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.out /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 s1opt.in /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.out s1opt.out.plots
else
$QC/bin/qchem -np 1 s1opt.in /data/mewes1/CALC/TADF/ANDBA-NaMe/s1opt.out
fi

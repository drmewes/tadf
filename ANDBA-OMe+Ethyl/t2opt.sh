...
#SBATCH --job-name=.data.mewes.CALC.TADF.ANDBA-OMe+Ethyl.t2opt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.in /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.out ] && cp /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.out /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 t2opt.in /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.out t2opt.out.plots
else
$QC/bin/qchem -np 1 t2opt.in /data/mewes/CALC/TADF/ANDBA-OMe+Ethyl/t2opt.out
fi

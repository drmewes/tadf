...
#SBATCH --job-name=dCALC.TADF_OLD.Yersin.GSOPT+PCM.gsopt_tol.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.out ] && cp /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.out /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save gsopt_tol.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.out gsopt_tol.out.plots
else
$QC/bin/qchem gsopt_tol.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_tol.out
fi

...
#SBATCH --job-name=dCALC.TADF_OLD.Yersin.GSOPT+PCM.gsopt_chx.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.out ] && cp /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.out /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save gsopt_chx.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.out gsopt_chx.out.plots
else
$QC/bin/qchem gsopt_chx.in /data/mewes/CALC/TADF_OLD/Yersin/GSOPT+PCM/gsopt_chx.out
fi

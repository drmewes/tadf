...
#SBATCH --job-name=dCALC.TADF_OLD.Yersin.s1opt_2.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.in /data/mewes/CALC/TADF_OLD/Yersin/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.out ] && cp /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.out /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 s1opt_2.in /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.out s1opt_2.out.plots
else
$QC/bin/qchem -np 1 s1opt_2.in /data/mewes/CALC/TADF_OLD/Yersin/s1opt_2.out
fi

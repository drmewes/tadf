...
#SBATCH --job-name=dCALC.TADF_OLD.Yersin.tleaopt.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.in /data/mewes/CALC/TADF_OLD/Yersin/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.out ] && cp /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.out /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 tleaopt.in /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.out tleaopt.out.plots
else
$QC/bin/qchem -np 1 tleaopt.in /data/mewes/CALC/TADF_OLD/Yersin/tleaopt.out
fi

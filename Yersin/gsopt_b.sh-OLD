...
#SBATCH --job-name=dCALC.TADF_OLD.Yersin.gsopt_b.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.in /data/mewes/CALC/TADF_OLD/Yersin/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.out ] && cp /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.out /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.out_OLD

# Execute the program
if [ "" = "yes" ] ; then
$QC/bin/qchem -save -np 1 gsopt_b.in /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.out gsopt_b.out.plots
else
$QC/bin/qchem -np 1 gsopt_b.in /data/mewes/CALC/TADF_OLD/Yersin/gsopt_b.out
fi

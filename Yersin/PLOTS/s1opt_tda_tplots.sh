...
#SBATCH --job-name=dCALC.TADF.Yersin.PLOTS.s1opt_tda_tplots.in
#SBATCH -t 3-0:00
#SBATCH -n 8
#SBATCH -N 1
...
# Go into Scratch
cd $SCRATCH
cp /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.in /data/mewes/CALC/TADF/Yersin/PLOTS/*.mol .

# Backup old out file is existing 
[ -e /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.out ] && cp /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.out /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.out_OLD

# Execute the program
if [ "yes" = "yes" ] ; then
$QC/bin/qchem -save s1opt_tda_tplots.in /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.out s1opt_tda_tplots.out.plots
else
$QC/bin/qchem s1opt_tda_tplots.in /data/mewes/CALC/TADF/Yersin/PLOTS/s1opt_tda_tplots.out
fi
